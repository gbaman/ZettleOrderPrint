...
def create_label_image(purchase: models.Purchase):
    for outer_prodict_id, outer_product in enumerate(purchase.products_purchased):
        img = Image.new('L', (696, 300 + (100*len(purchase.products_purchased))), color='white')
        ...
        for product_id, product in enumerate(purchase.products_purchased):
            if outer_prodict_id == product_id:
                d.rectangle((5, 250 + (product_id * 90), img.size[0] - 5, 350 + (product_id * 90)), outline="black", width=10)

            d.text((20, 260 + (product_id * 90)), f"- {product.product_name}", fill="black", font=main_font)
            if product.product_variations:
                d.text((40, 300 + (product_id * 90)), f"{product.clean_product_variations}", fill="black", font=ImageFont.truetype("arial.ttf", 30))
